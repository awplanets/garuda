...
function shipAccel()
{
	...
	return ship.ACC * (.94 + Math.min(.96, level * .24));
}
...
{
	bossHealth.className = 'boss-health active';
...
// A bullet is a straight vertical energy bolt.
bullets.image = render(function(c, a)
{
	var x = c.width / 2;
	a.lineCap = 'round';
	a.shadowBlur = scaled(10);
	a.shadowColor = '#F80';
	a.strokeStyle = '#F60';
	a.lineWidth = scaled(12);
	a.beginPath();
	a.moveTo(x, scaled(8));
	a.lineTo(x, c.height - scaled(8));
	a.stroke();
	a.shadowBlur = scaled(4);
	a.strokeStyle = '#FFD27A';
	a.lineWidth = scaled(4);
	a.beginPath();
	a.moveTo(x, scaled(8));
	a.lineTo(x, c.height - scaled(8));
	a.stroke();
...
{
	return render(function(c, a)
	{
		var r = c.width / 2;
		var g = a.createRadialGradient(r * .42, r * .38, 1, r, r, r);
		g.addColorStop(0, '#FFF8A8');
		g.addColorStop(.28, '#FFB02E');
		g.addColorStop(.62, '#F04A12');
		g.addColorStop(1, 'rgba(80,0,0,0)');
		a.fillStyle = g;
		a.beginPath();
		a.arc(r, r, r - 1, 0, Math.PI * 2);
		a.fill();
		a.shadowBlur = 8;
		a.shadowColor = '#F40';
		a.fillStyle = '#FF6A00';
		a.beginPath();
		a.arc(r, r, r * .55, 0, Math.PI * 2);
		a.fill();
	}, torpedos.R * 2);
}
...
{
	if (landingVideo && landingVideo.readyState >= 2)
...
function updateHud()
{
	...
	if (boss)
	{
		...
		bossHealth.className = 'boss-health active' + (boss.shield > 0 ? ' shielded' : '');
		bossHealth.setAttribute('aria-hidden', 'false');
...
function gameloop()
{
	...
	for (var i = hitEffects.length; i--; )
	{
		...
		a.lineWidth = scaled(2);
		a.beginPath();
		a.moveTo(scaled(-10) * p, 0);
		a.lineTo(scaled(10) * p, 0);
		a.moveTo(0, scaled(-10) * p);
		a.lineTo(0, scaled(10) * p);
		a.stroke();
		a.fillStyle = '#F80';
		a.beginPath();
		a.arc(0, 0, scaled(5) * p, 0, Math.PI * 2);
		a.fill();
		a.restore();
		if (!--h.t)
			hitEffects.splice(i, 1);
	...
	for (var i = enemies.length; i--; )
	{
		...
		a.drawImage(enemyImage, -enemies[i].r, enemies[i].y - y - enemies[i].r,
			enemies[i].r * 2, enemies[i].r * 2);
		a.restore();
		...
		else if (ship.y < enemies[i].y + d && ship.y > enemies[i].y - d &&
			ship.x > enemies[i].x - d && ship.x < enemies[i].x + d)
		{
			var canCrashHurt = ship.timeout < 0;
			hurt(35);
			if (canCrashHurt)
			{
				hitSpark((ship.x + enemies[i].x) / 2, (ship.y + enemies[i].y) / 2);
				explode((ship.x + enemies[i].x) / 2, (ship.y + enemies[i].y) / 2, enemies[i].r);
			}
		}
